...
`?=` (`wuttis`, `%wtts`) fishes the leg at wing `p` for tile `q`,
producing & (yes) if and only if leg `p` is in tile `q`.
...
of generating specific twigs that do useful things to nouns.
...
And even if this is/was not the case, anyone creating complex
forked types should use one of these standard structures - simply
because it is never really worth varying from this set of patterns, 
even if small structural efficiencies can be extracted.
...
For example, when we define a gate like `++tile`, defined as the
bush above (remember that when we use a tile as a twig, we get
the clam, ie, the normalizing gate) `tile` is just an arm which
produces a gate.  Nothing has any idea that this gate is built
from a tile.

So when we parse `[p=tile q=tile]` as a tile, the parser builds
the noun

    [[%bark %p %herb %cnzy %tile] [%bark %q %herb %cnzy %tile]]

In other words, `tile` in `p=tile` is actually a *twig*, but this
twig is a normalizing gate generated by clamming a tile.  In
short - it's totally herbaceous.

###[%weed p=twig]###

`%weed` is the lamest kind of tile - one which we define simply
by producing an *example* of the presumptive type.
