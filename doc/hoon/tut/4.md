...
In your Earth life, you learned about functions, among other
things.  The closest thing in Hoon is the `gate`.  
...
In the case of a gate, the payload is always a cell, arbitrarily 
labeled `sample` and `context`.  

    [battery [sample context]]

In a gate, the battery is exactly one arm - whose name is `$`, `buc` or
`blip` - ie, nothing.  

Remember again, the name is a symbol which is bound to an axis -- 
in this case the axis that contains our single formula.

One `arm`, one `twig`, one `formula`, and a `payload` consisting of `sample` and 
`context`.
...
feature -- code re-use, parameterized with inputs and providing a 
resulting value.

We will call this `deq` (there already being a perfectly usable 
`dec`), and we will write the code which invokes that function as

    (deq a)

To simplify things, we will write the code in the following form:
...
As the comments note, pay no attention for now to the 'boilerplate'.

This `XXXXX` twig, whatever it is, must extend the subject to
contain a core which includes an arm which produces our gate.

That is to say, whatever our definition looks like, it will involve
a core with an arm which produces the gate to be applied to the 
argument '`a`.  `[a ~]` is the 'sample' of that gate.  

The hoon concept of 'sample' and the Earth concept 'argument' are 
related in a similar way that 'gate' and 'function' are.  

If you are inclined to peek behind the "boilerplate curtain" you may 
note a resemblance between part of the third line and the `sample`, `[a ~]`.

Extend the subject (`=>`) to contain a core (`|%`) which includes an 
...
Well, it was ... just as it says above.  All of `zuse.hoon`
