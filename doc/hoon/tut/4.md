...
In the case of a `gate`, the `payload` is always a cell, whose 
...
Remember again, the name is a symbol which is bound to an axis, and an axis
...
fundamental semantic primitive.  It is provided by the language as an *atomic component*.

But gates - Hoon's equivalent of functions - are no more than a design pattern: a convention.
...
to ... do what we want... call the function `deq`? 
...
be 'run', we name it as an arm and can then refer to it 
'deeper down the rabbit hole'.

So, we use a `core` which includes an `arm` which produces our `gate` -- the function 
...
argument `a`.  `[a ~]` is going to be the `payload` of that eventual `gate`.
...
What do we name this `arm`?  How about... `deq`?  Yes-  `deq` it is.
...
linking against, if that helps -- it's not right, but it's analogous.
