...
####Tree address
`++axis` produces the [clam]() of an atom.
...
####Bit size
`++bloq` produces the [clam]() of an atom.
...
####Just a number
`++numb` produces the [clam]() of an atom.
...
####Public key
`++pass` produces the [clam]() of an atom.
...
####Private key
`++ring` produces the [clam]() of an atom.
...
####Atom format
`++odor` produces the [clam]() of a `@ta`; a text atom. See also, [odors]().
...
####UTF-8 byte
`++char` produces the [clam]() of a `@tD`. See also, [odors]().
...
####Text atom
`++cord` produces the [clam]() of a `@t`; a text atom. See also, [odors]().
...
####Text atom
`++span` produces the [clam]() of a `@ta`; a text atom. See also, [odors]().
...
####ASCII subset
`++term` produces the [clam]() of a `@tas`; a text atom. See also, [odors]().
...
####Parsed date
`++date` produces the [clam]() of a tuple containing an AD/BC loobean and year cell, a month atom and a [`++tarp`](#++tarp) (a parsed time).
