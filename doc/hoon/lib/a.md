...
++add is a jetted arm (~/, jets, link), which creates a dry %gold gate (link) (|=, link), accepting a sample of a pair of atoms, labeled 'a' and 'b' (^=, link). The product of the gate is cast to an atom (^-, %axil, link). If 'a' is equal to 0, the gate produces b. Else, the '$' arm of the gate is activated (%=, link), with the value of 'a' set to (dec a) (%-, dec, link), and 'b' set to +(b) (.+, link).  
